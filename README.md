...
Changing an address proves both mailboxes, in that order. `POST /api/v1/me/email/challenge` mails a code to the address on the session and only ever to that address; `POST /api/auth/email-otp/request-email-change` spends it and mails a second code to the new address; `POST /api/auth/email-otp/change-email` spends that one and the change lands. The current-mailbox step is what stops a stolen session moving an account somewhere its holder cannot reach - Better Auth's middleware on those routes only checks that a session exists, never that it is fresh. Volt mints the first code itself rather than exposing the plugin's own sender, which takes any address from an unauthenticated caller.

Archiving is reversible for a window and irreversible after it. The archive soft-deletes the organisation, its memberships, its invitations and all working simulation data in one transaction, stamping every row with the same instant. Within `RETENTION_WINDOW_DAYS` (default `30`) the owner can undo it through `POST /api/v1/organisations/:organisationId/restore`, which revives exactly the rows the archive took - a membership removed *before* the archive stays removed, and revoked invitations are not resurrected. After the window the worker's maintenance sweep hard-deletes the working data: intervals, summaries and runs, all of it synthetic and replayable from its seed. Ledger events, audit events, and the organisation and membership rows are never purged - the first two are evidence, and the last two are what that evidence references. See [ADR 0016](docs/adr/0016-retention-recovery-and-purge.md).

Account self-management belongs to the account holder alone; there is no administrator who can act for someone else, and none will be added without replacing [ADR 0011](docs/adr/0011-no-cross-tenant-administration.md). `DELETE /api/v1/me` closes the caller's own account: memberships are released and audited into each organisation they left, sessions and stored credentials are deleted so the address cannot be signed in with, and the user record is anonymised in place rather than erased. It is refused with `409 ACCOUNT_OWNS_ORGANISATIONS` while the account still owns an organisation, because an owner membership cannot be removed and an organisation with no owner is one nobody can administer while it still holds settlement records - transfer ownership or archive first. Ledger events are left exactly as they are: they carry `actorUserId` and are hash-linked, so rewriting one to drop a reference would break the chain that makes the ledger tamper-evident. What survives a closure is an opaque identifier attached to energy and money; what does not survive is any way to connect it to a person.
